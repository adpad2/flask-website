...
import torch
import numpy as np
from api.projects.athlete_progan.face_restorer.sr_model.rrdbnet_arch import RRDBNet
...
class RealESRNet(object):
    ...
    def load_srmodel(self, base_dir, model):
        ...
        if model is None:
            loadnet = torch.load('api/projects/athlete_progan/face_restorer/models/realesrnet_x%d.pth' % self.scale)
        else:
            loadnet = torch.load('api/projects/athlete_progan/face_restorer/models/%s_x%d.pth' % (model, self.scale))
